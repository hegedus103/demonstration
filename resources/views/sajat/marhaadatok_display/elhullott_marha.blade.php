...
<div class="btn-group btn-group-toggle" data-toggle="buttons">
  ...
  <label class="btn btn-primary btn-sm active">
    <input type="radio" name="options" id="{{$option[2]='option3'}}" onchange="window.location.href='{{ route("elhullott_marha") }}'"  autocomplete="off" checked> Elhullottak
  </label>
   <label class="btn btn-primary btn-sm">
...
<tr class="kiiras-fejlec">
        ...
        <tr>
            ...
            <td>
                    <a title="Az elhullas visszavonasa itt lehetséges" class="kiiras-href" href="{{ route('elhullas_visszavonasa',['id' =>$row->id]) }}">{{$row->enarszam}}</a>
                </td>
                    <td>{{$row->neve}}</td>
                    <td>{{$row->neme}}</td>
                    ...
                    <td>{{$row->bekerult}}</td>
                    <td>
                        @if($row->elhullas_datuma=='1000-01-01')
