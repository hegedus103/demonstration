...
<template>
  <div class="bevitel-dialog">
    <div class="bevitel-fejlec">Marha törzsadat módosítás</div>
  ...
  <span class="bevitel-error-szine" v-show="!validations.enarszam.is_valid">{{ validations.enarszam.text }}</span>
            ...
            <input type="text" title="Az enárszám pontosan egy tízjegyű szám legyen!"class="'col-lg-4 control-label'" name="enarszam" id="enarszam" v-model="AllatokMarha.enarszam" value="AllatokMarha->enarszam" />
  </div>
        <div class="form-group">
  ...
  <span class="bevitel-error-szine" v-show="!validations.neve.is_valid">{{ validations.neve.text }}</span>
            ...
            <input type="text" title="Ha nem ír be semmmit akkor az enarszamot irjuk be a nevének"class='col-lg-4 control-label' name="neve" id="neve" v-model="AllatokMarha.neve" value="AllatokMarha->neve"/>
        </div>
     <div class="form-group">
 <div>
...
<div class="form-group">
  ...
  <button class="btn btn-success" v-on:click="CancelMarhatorzsUpdate">Cancel</button>
...
export default {
  ...
  methods: {
     ...
     // alert('  a lekert  :'+this.getroute);
      axios.get(this.getroute).then(response => 
      {
         currentObject.output= response.data;
        // this.items = response.data;
   
         this.AllatokMarha.enarszam=response.data[0]['enarszam'];
         this.AllatokMarha.neve=response.data[0]['neve'];
         this.AllatokMarha.neme=response.data[0]['neme'];
         this.AllatokMarha.szuletes_datuma=response.data[0]['szuletes_datuma'];
         this.AllatokMarha.bekerult=response.data[0]['bekerult'];
         this.AllatokMarha.fajta=response.data[0]['fajta'];
         this.AllatokMarha.szine=response.data[0]['szine'];
         this.AllatokMarha.anya_enarszam=response.data[0]['anya_enarszam'];
         this.AllatokMarha.anya_neve=response.data[0]['anya_neve'];
         this.AllatokMarha.jarlat_sorszam=response.data[0]['jarlat_sorszam'];
         this.AllatokMarha.jarlat_kiadasa=response.data[0]['jarlat_kiadasa'];
         this.AllatokMarha.szarmazas_tenyeszet=response.data[0]['szarmazas_tenyeszet'];
         //alert('a lekert enarszam:'+this.items[0]['enarszam']);
       }
      ).catch(response => {
                    // Error Handling
                  });
...
//--------------------------------------------------------------
 ...
 if( SajatErrors.neve[0].length>0)
                  {
                       this.validations.neve.is_valid = false;
    this.validations.neve.text =SajatErrors.neve[0];
    this.validations.eredmeny=false; 
                  }// vege ha neve
