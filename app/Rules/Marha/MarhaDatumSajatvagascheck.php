...
//use App\Sajatlib\utils;
class MarhaDatumSajatvagascheck implements Rule
{
    /**
    ...
    public function passes($attribute, $value)
    {
        //
          ...
          //print_r($data);exit;
              $sajat_vagas=$this->Sbol_szam($data['sajat_vagas_datuma']);
        $bekerult=$this->Sbol_szam($data['bekerult']);
        $szuletes=$this->Sbol_szam($data['szuletes_datuma']);
        if($sajat_vagas<$szuletes)
        {
            $this->melyik=1;
            return false;
        }
        if($sajat_vagas<$bekerult)
        {
            $this->melyik=2;
            return false;
        }
    ...
    {
          if($this->melyik==1)
            return 'A saját vágás kisebb mint a születés dátuma!';
        if($this->melyik==2)
            return 'A saját vágás kisebb mint a bekerülés dátuma!';
        return 'A sajat vágás dátuma nem megfelelő!';
    }
    //************************************************************
/**
