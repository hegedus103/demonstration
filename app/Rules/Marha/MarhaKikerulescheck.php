...
class MarhaKikerulescheck implements Rule
{
    /**
    ...
    public function passes($attribute, $value)
    {
        //
          ...
          //print_r($data);
         $kikerult=$this->Sbol_szam($data['kikerules_datuma']);
        $bekerult=$this->Sbol_szam($data['bekerult']);
        $szuletes=$this->Sbol_szam($data['szuletes_datuma']);
        if($kikerult<$bekerult)
        {
            //echo'A kikerules:'.$kikerult.'  a bekerules:'.$bekerult;exit;
            $this->melyik=1;
            return false;
        }
        if($kikerult<$szuletes)
        {
            //echo'A kikerules:'.$kikerult.'  a bekerules:'.$szuletes;exit;
            $this->melyik=2;
            return false;
        }
   ...
   public function message() 
    {
        if ($this->melyik==2) {
            return 'A kikerülés dátuma kisebb mint a születés dátuma!';
        }
        if ($this->melyik==1) {
            return 'A kikerülés dátuma kisebb mint a bekerülés dátuma!';
        }
     
        return 'A kikerülés dátuma nem megfelelő!';
    }
