...
class MarhaDatumelhullascheck implements Rule
{
    /**
    ...
    public function passes($attribute, $value )
    {
        ...
        //return true;
         ...
         $elhullas=$this->Sbol_szam($data['elhullas_datuma']);
        $bekerult=$this->Sbol_szam($data['bekerult']);
        $szuletes=$this->Sbol_szam($data['szuletes_datuma']);
        /*
        echo'<br>A kiszamitott elhullas:'.$elhullas;
        echo'<br>A kiszamitott bekerulet:'.$bekerult;
        echo'<br>A kiszamitott szuletes:'.$szuletes.'<br>';
        print_r($data);exit; 
        */
        if($elhullas<$szuletes)
        {
            $this->melyik=1;
            return false;
        }
        if($elhullas<$bekerult)
        {
            $this->melyik=2;
            return false;
        }
    ...
    {
        if($this->melyik==1)
            return 'Az elhullas kisebb mint a születés dátuma!';
        if($this->melyik==2)
            return 'Az elhullas kisebb mint a bekerülés dátuma!';
        return 'Az elhullas dátuma nem megfelelő!';
    }
//************************************************************
/**
